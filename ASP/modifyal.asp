...
<body>
	...
	<section class="page-section">
		<div class="container">
			...
			rs1.open "SELECT * FROM Albums WHERE aid = " + request.form("aid") + " ", conn1
			
			%>
			<form class="contact-form" method = post action = updateal.asp?aid=<% = rs1("aid")%>>
			<input type = hidden name=aid value = <% = rs1("aid")%>>
				<div class="row">
					<div class="col-md-3">
						<input type="number" disabled=true value= <% = rs1("aid") %>>
					</div>
					<div class="col-md-3">
						<input type="text" name=aname value= <% = "'" + rs1("aname") + "'" %>>
					</div>
					<div class="col-md-3">
						<input type="number" name=bandid value= <% = rs1("bandid") %>>
					</div>
					<div class="col-md-3">
						<input type="number" name=artistid value= <% = rs1("artistid") %>>
					</div>
					<div class="col-md-3">
						<input type="text" name=pic value= <% = rs1("pic") %>>
					</div>
					<div class="col-md-3">
						<input type="text" name=length value= <% = rs1("length") %>>
					</div>
					<div class="col-md-3">
						<input type="date/time" name=released value= <% = rs1("released") %>>
					</div>
					<div class="col-md-12">
						<input type=text value= <% = rs1("adescription") %> name=desc>
					</div>
				</div>
