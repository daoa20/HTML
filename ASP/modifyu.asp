...
<body>
	...
	<section class="page-section">
		<div class="container">
			...
			rs1.open "SELECT * FROM Users WHERE id = " + request.form("id") + " ", conn1
			
			%>
			<form class="contact-form" method = post action = updateu.asp>
			<input type = hidden name=id value = <% = rs1("id")%>>
				<div class="row">
					<div class="col-md-3">
						<input type="number" disabled=true value= <% = rs1("id") %>>
					</div>
					<div class="col-md-3">
						<input type="text" name=username value= <% = "'" + rs1("username") + "'" %>>
					</div>
					<div class="col-md-3">
						<input type="text" name=password value= <% = rs1("password") %>>
					</div>
					<div class="col-md-3">
