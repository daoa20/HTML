...
<body>
	...
	<section class="page-section">
		<div class="container">
			...
			rs1.open "SELECT * FROM Playlists WHERE pid = " + request.form("pid") + " ", conn1
			
			%>
			<form class="contact-form" method = post action = updateps.asp>
			<input type = hidden name=pid value = <% = rs1("pid")%>>
				<div class="row">
					<div class="col-md-4">
						<input type="number" disabled=true value= <% = rs1("pid") %>>
					</div>
					<div class="col-md-4">
						<input type="text" name=pname value= <% = "'" + rs1("pname") + "'" %>>
					</div>
					<div class="col-md-4">
						<input type="text" name=pic value= <% = rs1("pic") %>>
					</div>
					<div class="col-md-12">
						<input type=text value= <% = rs1("pdesc") %> name=pdesc>
					</div>
				</div>
