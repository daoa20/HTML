...
<body>
	...
	<section class="page-section">
		<div class="container">
			...
			rs1.open "SELECT * FROM Songs WHERE id = " + request.form("ids") + " ", conn1
			
			%>
			<form method = post action = update.asp?num=<% = rs1("id")%>>
			<input type = hidden name=ids value = <% = rs1("id")%>>
				<div class="row">
					<div class="col-md-4">
						<input type="number" disabled=true value= <% = rs1("id") %>>
					</div>
					<div class="col-md-4">
						<input type="text" name=name value= <% = rs1("name") %>>
					</div>
					<div class="col-md-4">
						<input type="number" name=albumid value= <% = rs1("albumid") %>>
					</div>
					<div class="col-md-12">
