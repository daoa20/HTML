...
<body>
	...
	<section class="page-section">
		<div class="container">
			...
			rs1.open "SELECT * FROM Bands WHERE bid = " + request.form("bid") + " ", conn1
			
			%>
			<form class="contact-form" method = post action = updateb.asp?bid=<% = rs1("bid")%>>
			<input type = hidden name=bid value = <% = rs1("bid")%>>
				<div class="row">
					<div class="col-md-3">
						<input type="number" disabled=true value= <% = rs1("bid") %>>
					</div>
					<div class="col-md-3">
						<input type="text" name=bname value= <% = "'" + rs1("bname") + "'" %>>
					</div>
					<div class="col-md-3">
						<input type="number" name=genreid value= <% = rs1("genreid") %>>
					</div>
				</div>
